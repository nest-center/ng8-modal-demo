import { NgModule, Component, HostListener, OnInit, AfterViewInit, AfterViewChecked, Directive, Optional, AfterContentInit, Input, Output, EventEmitter, ElementRef, ContentChildren, TemplateRef, QueryList, ViewChild, NgZone, OnDestroy } from '@angular/core';
import { CommonModule } from '@angular/common';
import { Injectable } from '@angular/core';
import { Subject, Subscription } from 'rxjs';

import { Column, PrimeTemplate, ComponentsModule } from '../components.module';
import { DomUtils } from '../../utils/dom.utils';
import { ObjectUtils } from '../../utils/object.utils';
import { ISortMeta, ITableState, IFilterMetadata, IBlockTableUI } from '../components.interface';
    ...
    template : `
		<div #container [ngStyle]="style" [class]="styleClass"
		     [ngClass]="{'ui-table ui-widget': true, 'ui-table-responsive': responsive, 'ui-table-resizable': resizableColumns,
                ...
                'table-operation': operationTable,
			...
			<div class="ui-table-wrapper" *ngIf="!scrollable">
				<table #table class="table" [ngClass]="tableStyleClass" [ngStyle]="tableStyle">
			...
			<div class="ui-table-scrollable-wrapper" *ngIf="scrollable">
				<div class="ui-table-scrollable-view ui-table-frozen-view" *ngIf="frozenCols" [pScrollableView]="frozenColumns" [ngStyle]="frozenDirection" [frozen]="true" [scrollHeight]="scrollHeight"></div>
...
export class Table implements OnInit, AfterViewInit, AfterContentInit, IBlockTableUI, OnDestroy {

    frozenColumns : any = [];
    @Input() frozenValue : any[];
    @Input() frozenCols : any;
    @Input() style : any;
    @Input() frozenFloat : any;
    frozenDirection : any = {};

    @Input() styleClass : string;

    @Input() operationTable : boolean;
    ...
    @Input() filters : { [s : string] : IFilterMetadata; } = {};
    ...
    ngOnInit () {
        ...
        setTimeout(() => {
            if (this.frozenCols) {
                this.frozenColumns = this.frozenCols.frozen === 'right' ? [this.frozenCols, ...this.columns] : [...this.columns, this.frozenCols];
                this.frozenDirection = this.frozenCols.frozenDirection;
                // this.columns.push(this.frozenCols);
                this.columns = this.frozenCols.frozen === 'right' ? [...this.columns, this.frozenCols] : [this.frozenCols, ...this.columns];
            }
    ...
    @Input() get multiSortMeta () : ISortMeta[] {
        return this._multiSortMeta;
    }

    set multiSortMeta (val : ISortMeta[]) {
    ...
    getSortMeta (field : string) {
        if (this.multiSortMeta && this.multiSortMeta.length) {
            for (const sortMeta of this.multiSortMeta) {
    ...
    handleRowClick (event) {
        ...
        if (targetNode === 'INPUT' || targetNode === 'BUTTON' || targetNode === 'A' ||
            ...
            (DomUtils.hasClass(event.originalEvent.target, 'ui-clickable'))) {
            return;
        }
                ...
                for (const val of this.value) {
                    ...
                    for (const prop in this.filters) {
                        if (this.filters.hasOwnProperty(prop) && prop !== 'global') {
                            ...
                            const dataFieldValue = ObjectUtils.resolveFieldData(val, filterField);
                    ...
                    if (this.filters['global'] && !globalMatch && globalFilterFieldsArray) {
                        for (const globalFilterFields of globalFilterFieldsArray) {
                            const globalFilterField = globalFilterFields.field || globalFilterFields;
                            globalMatch = this.filterConstraints[this.filters['global'].matchMode](ObjectUtils.resolveFieldData(val, globalFilterField), this.filters['global'].value);
                            if (globalMatch) {
                                break;
                            }
                        }
    ...
    filterConstraints = {
        ...
        in (value, filter : any[]) : boolean {
            ...
            for (const item of filter) {
                if (item === value || (value.getTime && item.getTime && value.getTime() === item.getTime())) {
                    return true;
                }
            }
    ...
    isEditingCellValid () {
        return (this.editingCell && DomUtils.find(this.editingCell, '.ng-invalid.ng-dirty').length === 0);
    }
        ...
        if (!this.documentEditListener) {
            this.documentEditListener = (event) => {
                if (this.editingCell && !this.editingCellClick && this.isEditingCellValid()) {
                    DomUtils.removeClass(this.editingCell, 'ui-editing-cell');
    ...
    saveRowEdit (rowData : any, rowElement : HTMLTableRowElement) {
        if (DomUtils.find(rowElement, '.ng-invalid.ng-dirty').length === 0) {
            const dataKeyValue = String(ObjectUtils.resolveFieldData(rowData, this.dataKey));
    ...
    onColumnResizeBegin (event) {
        const containerLeft = DomUtils.getOffset(this.containerViewChild.nativeElement).left;
    ...
    onColumnResize (event) {
        const containerLeft = DomUtils.getOffset(this.containerViewChild.nativeElement).left;
        DomUtils.addClass(this.containerViewChild.nativeElement, 'ui-unselectable-text');
    ...
    onColumnResizeEnd (event, column) {
                ...
                if (nextColumn) {
                    ...
                    if (newColumnWidth > 15 && nextColumnWidth > parseInt(nextColumnMinWidth, 10)) {
                        if (this.scrollable) {
                            ...
                            const scrollableBodyTable = DomUtils.findSingle(scrollableView, 'table.ui-table-scrollable-body-table');
                            const scrollableHeaderTable = DomUtils.findSingle(scrollableView, 'table.ui-table-scrollable-header-table');
                            const scrollableFooterTable = DomUtils.findSingle(scrollableView, 'table.ui-table-scrollable-footer-table');
                            const resizeColumnIndex = DomUtils.index(column);
        ...
        DomUtils.removeClass(this.containerViewChild.nativeElement, 'ui-unselectable-text');
    }

    findParentScrollableView (column) {
        if (column) {
            let parent = column.parentElement;
            while (parent && !DomUtils.hasClass(parent, 'ui-table-scrollable-view')) {
    ...
    onColumnDragStart (event, columnElement) {
        this.reorderIconWidth = DomUtils.getHiddenElementOuterWidth(this.reorderIndicatorUpViewChild.nativeElement);
        this.reorderIconHeight = DomUtils.getHiddenElementOuterHeight(this.reorderIndicatorDownViewChild.nativeElement);
    ...
    onColumnDragEnter (event, dropHeader) {
        if (this.reorderableColumns && this.draggedColumn && dropHeader) {
            event.preventDefault();
            const containerOffset = DomUtils.getOffset(this.containerViewChild.nativeElement);
            const dropHeaderOffset = DomUtils.getOffset(dropHeader);

            if (this.draggedColumn !== dropHeader) {
                const dragIndex = DomUtils.indexWithinGroup(this.draggedColumn, 'preorderablecolumn');
                const dropIndex = DomUtils.indexWithinGroup(dropHeader, 'preorderablecolumn');
    ...
    onColumnDrop (event, dropColumn) {
        ...
        if (this.draggedColumn) {
            const dragIndex = DomUtils.indexWithinGroup(this.draggedColumn, 'preorderablecolumn');
            let dropIndex = DomUtils.indexWithinGroup(dropColumn, 'preorderablecolumn');
    ...
    onRowDragOver (event, index, rowElement) {
        if (this.rowDragging && this.draggedRowIndex !== index) {
            const rowY = DomUtils.getOffset(rowElement).top + DomUtils.getWindowScrollTop();
            const pageY = event.pageY;
            const rowMidY = rowY + DomUtils.getOuterHeight(rowElement) / 2;
            const prevRowElement = rowElement.previousElementSibling;

            if (pageY < rowMidY) {
                DomUtils.removeClass(rowElement, 'ui-table-dragpoint-bottom');

                this.droppedRowIndex = index;
                if (prevRowElement)
                    DomUtils.addClass(prevRowElement, 'ui-table-dragpoint-bottom');
                else
                    DomUtils.addClass(rowElement, 'ui-table-dragpoint-top');
            } else {
                if (prevRowElement)
                    DomUtils.removeClass(prevRowElement, 'ui-table-dragpoint-bottom');
                else
                    DomUtils.addClass(rowElement, 'ui-table-dragpoint-top');

                this.droppedRowIndex = index + 1;
                DomUtils.addClass(rowElement, 'ui-table-dragpoint-bottom');
            }
        }
    }
    ...
    onRowDragLeave (event, rowElement) {
        ...
        DomUtils.removeClass(rowElement, 'ui-table-dragpoint-bottom');
        DomUtils.removeClass(rowElement, 'ui-table-dragpoint-top');
    }
    ...
    saveState () {
        ...
        if (Object.keys(state).length && this.stateKey) {
    ...
    restoreState () {
        ...
        if (stateString) {
            const state : ITableState = JSON.parse(stateString);
    ...
    saveColumnWidths (state) {
        const widths = [];
        const headers = DomUtils.find(this.containerViewChild.nativeElement, '.ui-table-thead > tr:first-child > th');
        headers.map(header => widths.push(DomUtils.getOuterWidth(header)));
        state.columnWidths = widths.join(',');

        if (this.columnResizeMode === 'expand') {
            state.tableWidth = this.scrollable ? DomUtils.findSingle(this.containerViewChild.nativeElement, '.ui-table-scrollable-header-table').style.width :
                DomUtils.getOuterWidth(this.tableViewChild.nativeElement) + 'px';
        }
    }
    ...
    restoreColumnWidths () {
        if (this.columnWidthsState) {
            ...
            if (this.columnResizeMode === 'expand' && this.tableWidthState) {
                if (this.scrollable) {
                    const scrollableBodyTable = DomUtils.findSingle(this.containerViewChild.nativeElement, '.ui-table-scrollable-body-table');
                    const scrollableHeaderTable = DomUtils.findSingle(this.containerViewChild.nativeElement, '.ui-table-scrollable-header-table');
                    const scrollableFooterTable = DomUtils.findSingle(this.containerViewChild.nativeElement, '.ui-table-scrollable-footer-table');
            ...
            if (this.scrollable) {
                const headerCols = DomUtils.find(this.containerViewChild.nativeElement, '.ui-table-scrollable-header-table > colgroup > col');
                const bodyCols = DomUtils.find(this.containerViewChild.nativeElement, '.ui-table-scrollable-body-table > colgroup > col');

                headerCols.map((col, index) => col.style.width = widths[index] + 'px');
                bodyCols.map((col, index) => col.style.width = widths[index] + 'px');
            } else {
                const headers = DomUtils.find(this.tableViewChild.nativeElement, '.ui-table-thead > tr:first-child > th');
    ...
    template : `
		<div #scrollHeader class="ui-table-scrollable-header ui-widget-header">
			<div #scrollHeaderBox class="ui-table-scrollable-header-box">
				<table class="table ui-table-scrollable-header-table" [ngClass]="dt.tableStyleClass" [ngStyle]="dt.tableStyle">
		...
		<div #scrollBody class="ui-table-scrollable-body">
			<table #scrollTable class="table" [ngClass]="{'ui-table-scrollable-body-table': true, 'ui-table-virtual-table': dt.virtualScroll}" [class]="dt.tableStyleClass" [ngStyle]="dt.tableStyle">
				<ng-container *ngTemplateOutlet="frozen ? dt.frozenColGroupTemplate||dt.colGroupTemplate : dt.colGroupTemplate; context {$implicit: columns}"></ng-container>
		...
		<div #scrollFooter *ngIf="dt.footerTemplate" class="ui-table-scrollable-footer ui-widget-header">
			<div #scrollFooterBox class="ui-table-scrollable-footer-box">
				<table class="table ui-table-scrollable-footer-table" [ngClass]="dt.tableStyleClass" [ngStyle]="dt.tableStyle">
					<ng-container *ngTemplateOutlet="frozen ? dt.frozenColGroupTemplate||dt.colGroupTemplate : dt.colGroupTemplate; context {$implicit: columns}"></ng-container>
...
export class ScrollableView implements AfterViewInit, OnDestroy, AfterViewChecked {
    ...
    ngAfterViewInit () {
        if (!this.frozen) {
            if (this.dt.frozenColumns || this.dt.frozenBodyTemplate) {
                DomUtils.addClass(this.el.nativeElement, 'ui-table-unfrozen-view');
            }

            const frozenView = this.el.nativeElement.previousElementSibling;
            if (frozenView) {
                this.frozenSiblingBody = DomUtils.findSingle(frozenView, '.ui-table-scrollable-body');
            }
        } else {
            // this.scrollBodyViewChild.nativeElement.style.paddingBottom = DomUtils.calculateScrollbarWidth() + 'px';
            const scrollableView = this.el.nativeElement.nextElementSibling;
            if (scrollableView) {
                this.scrollableSiblingBody = DomUtils.findSingle(scrollableView, '.ui-table-scrollable-body');
            }
        }
    ...
    bindEvents () {
        this.zone.runOutsideAngular(() => {
            const scrollBarWidth = DomUtils.calculateScrollbarWidth();
    ...
    onBodyScroll (event) {
        ...
        if (this.dt.virtualScroll) {
            const viewport = DomUtils.getOuterHeight(this.scrollBodyViewChild.nativeElement);
            const tableHeight = DomUtils.getOuterHeight(this.scrollTableViewChild.nativeElement);
            const pageHeight = this.dt.virtualRowHeight * this.dt.rows;
            const virtualTableHeight = DomUtils.getOuterHeight(this.virtualScrollerViewChild.nativeElement);
    ...
    setScrollHeight () {
        if (this.scrollHeight && this.scrollBodyViewChild && this.scrollBodyViewChild.nativeElement) {
            if (this.scrollHeight.indexOf('%') !== -1) {
                ...
                const containerHeight = DomUtils.getOuterHeight(this.dt.el.nativeElement.children[0]);
                ...
                if (this.scrollHeight.includes('calc')) {
                    ...
                    relativeHeight = (DomUtils.getOuterHeight(this.dt.el.nativeElement.parentElement) * percentHeight / 100) - diffValue;
                } else {
                    relativeHeight = DomUtils.getOuterHeight(this.dt.el.nativeElement.parentElement) * parseInt(this.scrollHeight, 10) / 100;
                }
                ...
                if (this.frozen) {
                    scrollBodyHeight -= DomUtils.calculateScrollbarWidth();
                }
                ...
                if (this.frozen && this.scrollableSiblingBody && DomUtils.getOuterWidth(this.scrollableSiblingBody) < DomUtils.getOuterWidth(this.scrollableSiblingBody.children[0]))
                    this.scrollBodyViewChild.nativeElement.style.maxHeight = (parseInt(this.scrollHeight, 10) - DomUtils.calculateScrollbarWidth()) + 'px';
                else
    ...
    hasVerticalOverflow () {
        return DomUtils.getOuterHeight(this.scrollTableViewChild.nativeElement) > DomUtils.getOuterHeight(this.scrollBodyViewChild.nativeElement);
    }

    alignScrollBar () {
        if (!this.frozen) {
            const scrollBarWidth = this.hasVerticalOverflow() ? DomUtils.calculateScrollbarWidth() : 0;
...
@Component({
    selector : 'p-sortIcon',
    template : `
		<i class="icon-arrow" [ngClass]="{'active': sortOrder === -1}"></i>
		<i class="icon-sort-down" [ngClass]="{'active': sortOrder === 1}"></i>
    `
})
...
export class SelectableRow implements OnInit, OnDestroy {
    ...
    findNextSelectableRow (row : HTMLTableRowElement) : HTMLTableRowElement {
        ...
        if (nextRow) {
            if (DomUtils.hasClass(nextRow, 'ui-selectable-row'))
                return nextRow;
            else
    ...
    findPrevSelectableRow (row : HTMLTableRowElement) : HTMLTableRowElement {
        ...
        if (prevRow) {
            if (DomUtils.hasClass(prevRow, 'ui-selectable-row'))
                return prevRow;
            else
...
export class ResizableColumn implements AfterViewInit, OnDestroy {
    ...
    ngAfterViewInit () {
        if (this.isEnabled()) {
            DomUtils.addClass(this.el.nativeElement, 'ui-resizable-column');
...
export class ReorderableColumn implements AfterViewInit, OnDestroy {
    ...
    onMouseDown (event) {
        if (event.target.nodeName === 'INPUT' || event.target.nodeName === 'TEXTAREA' || DomUtils.hasClass(event.target, 'ui-column-resizer'))
...
export class EditableColumn implements AfterViewInit {
    ...
    ngAfterViewInit () {
        if (this.isEnabled()) {
            DomUtils.addClass(this.el.nativeElement, 'ui-editable-column');
        }
    }
        ...
        if (this.isEnabled()) {
            ...
            if (this.dt.editingCell) {
                if (this.dt.editingCell !== this.el.nativeElement) {
                    ...
                    DomUtils.removeClass(this.dt.editingCell, 'ui-editing-cell');
                    this.openCell();
                }
            } else {
                this.openCell();
    ...
    openCell () {
        ...
        DomUtils.addClass(this.el.nativeElement, 'ui-editing-cell');
        ...
        this.zone.runOutsideAngular(() => {
            setTimeout(() => {
                ...
                const focusableElement = DomUtils.findSingle(this.el.nativeElement, focusCellSelector);
    ...
    findCell (element) {
        if (element) {
            let cell = element;
            while (cell && !DomUtils.hasClass(cell, 'ui-editing-cell')) {
    ...
    moveToPreviousCell (event : KeyboardEvent) {
        ...
        if (targetCell) {
            DomUtils.invokeElementMethod(event.target, 'blur');
            DomUtils.invokeElementMethod(targetCell, 'click');
            event.preventDefault();
        }
    }
    ...
    findPreviousEditableColumn (cell : Element) {
        ...
        if (prevCell) {
            if (DomUtils.hasClass(prevCell, 'ui-editable-column'))
                return prevCell;
            else
    ...
    findNextEditableColumn (cell : Element) {
        ...
        if (nextCell) {
            if (DomUtils.hasClass(nextCell, 'ui-editable-column'))
                return nextCell;
            else
...
export class TableRadioButton implements OnInit, OnDestroy {
    ...
    onFocus () {
        DomUtils.addClass(this.boxViewChild.nativeElement, 'ui-state-focus');
    }

    onBlur () {
        DomUtils.removeClass(this.boxViewChild.nativeElement, 'ui-state-focus');
    }
...
@Component({
    ...
    template : `
		<div class="ui-checkbox ui-widget" (click)="onClick($event)">
			<div class="ui-helper-hidden-accessible">
				<input type="checkbox" [checked]="checked" (focus)="onFocus()" (blur)="onBlur()" [disabled]="disabled">
			</div>
			<div #box class="checkbox" [ngClass]="{'checked':checked, 'disabled':disabled}">
				<label></label>
			</div>
		</div>
    `
...
@Component({
    ...
    template : `
		<div class="ui-checkbox ui-widget" (click)="onClick($event)">
			...
			<div #box class="checkbox" [ngClass]="{'checked':checked, 'disabled': isDisabled()}">
				<label></label>
			</div>
		</div>
    `
...
export class ReorderableRowHandle implements AfterViewInit {
    ...
    ngAfterViewInit () {
        DomUtils.addClass(this.el.nativeElement, 'ui-table-reorderablerow-handle');
    }
}
...
export class ReorderableRow implements AfterViewInit {
    ...
    onMouseDown (event) {
        this.el.nativeElement.draggable = DomUtils.hasClass(event.target, 'ui-table-reorderablerow-handle');
    }
...
@NgModule({
    ...
    exports : [Table, ComponentsModule, SortableColumn, SelectableRow, RowToggler, ContextMenuRow, ResizableColumn, ReorderableColumn, EditableColumn, CellEditor, SortIcon, TableRadioButton, TableCheckbox, TableHeaderCheckbox, ReorderableRowHandle, ReorderableRow, SelectableRowDblClick, EditableRow, InitEditableRow, SaveEditableRow, CancelEditableRow],
