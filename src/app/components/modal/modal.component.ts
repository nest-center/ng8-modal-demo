import { ChangeDetectionStrategy, Compiler, Component, ComponentFactoryResolver, ComponentRef, ElementRef, Inject, OnDestroy, PLATFORM_ID, Renderer2, ViewChild, ViewContainerRef } from '@angular/core';
		...
		<div class="modal-scroll">
			<div class="modal-body" #modalBody>
				<div *ngIf="!isEmpty(config.title)" class="modal-header">
					...
					<span *ngIf="!isEmpty(config.titleDesc)" class="title-desc">{{ config.titleDesc }}</span>
				</div>
...
export class ModalComponent implements OnDestroy {
    ...
    protected readonly modalStyleId : string = 'modalStyle';

    @ViewChild('modalBody', { static : false }) protected modalBody : ElementRef;
    @ViewChild('modalComponent', { static : false }) protected modalComponent : ElementRef;

    protected eleListen : () => void;
    protected bodyListen : () => void;
    ...
    initModal () {
        if (isPlatformBrowser(this.platformId)) {
            ...
            if (this.config.modalContentStyles) {
                const modalContentDom = this.eleRef.nativeElement.querySelector('.modal-content');
                if (modalContentDom) {
                    this.setStyles('content', modalContentDom, this.config.modalContentStyles);
                }
            }
     ...
     * set modal style
     */
    protected setStyles (type : 'header' | 'body' | 'content', dom : any, styles : any) {
        for (const prop in styles) {
            if (styles.hasOwnProperty(prop)) {
                if (type === 'header' || type === 'content') {
    ...
    protected animateInit (isClose : boolean = false) {
     ...
     */
    protected setBodyAnimation (animateName : string, animateTiming : string, timingFun : string) {
     ...
     */
    protected modalBodyStyle (key : string, value : any) {
    ...
    protected _callback (callback : Function, data? : any) {
     ...
     * @param data
     */
    protected checkCloseWindow (callbackName : string, callback : Function, data? : any) {
        const result : any = this._callback(callback, data);
        if (result === true) {
            this.closeWindow();
            return;
        }
